...
if lrsca="999999999.9" then lrsca=".";
if lrsp="999999999.9" then lrsp=".";
RUN; 
...
KEEP timepointid haltid LFGF FGF PTH _25_OH_D _1_25_OH_D GENE1 AGE SEX RACEF3 RACE PKDAGE HPBAGE hght_cm WGHT_KG BMI BSA MARIT EMPL EDU;
RUN;
...
PROC TRANSPOSE DATA=CAP.HALT3 OUT=WORK.HALT4 (DROP= _NAME_ _LABEL_) PREFIX=lhttkv ;
BY HALTID;
ID timepointid;
VAR lhttkv;
RUN;

PROC TRANSPOSE DATA=CAP.HALT3 OUT=WORK.HALT5 (DROP= _NAME_ _LABEL_) PREFIX=lrsp ;
BY HALTID;
ID timepointid;
VAR lrsp;
RUN;
PROC TRANSPOSE DATA=CAP.HALT3 OUT=WORK.HALT6 (DROP= _NAME_ _LABEL_) PREFIX=lrsca ;
BY HALTID;
ID timepointid;
VAR lrsca;
RUN;

PROC TRANSPOSE DATA=CAP.HALT3 OUT=WORK.HALT7 (DROP= _NAME_ _LABEL_) PREFIX=ualbum ;
BY HALTID;
ID timepointid;
VAR ualbum;
RUN;
...
DATA CAP.transposed;
MERGE WORK.HALT WORK.HALT1 WORK.HALT2 WORK.HALT3 WORK.HALT4 WORK.HALT5 WORK.HALT6 work.halt7 CAP.CONSTANTS WORK.CHANGETKV  WORK.CHANGEEGFR; /*PUT ALL TRANSPOSED DATA SETS HERE*/;
BY HALTID;
RUN;
